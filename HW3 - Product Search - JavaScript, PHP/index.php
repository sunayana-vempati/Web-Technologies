...
{
	if($category!='all')
		$api_request='http://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsAdvanced&SERVICE-VERSION=1.0.0&SECURITY-APPNAME=key&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&paginationInput.entriesPerPage=20&keywords='.$keyword.'&categoryId='.$category.'&buyerPostalCode='.$zip.'&itemFilter(0).name=MaxDistance&itemFilter(0).value='.$distance.'&itemFilter(1).name=FreeShippingOnly&itemFilter(1).value='.$free.'&itemFilter(2).name=LocalPickupOnly&itemFilter(2).value='.$local.'&itemFilter(3).name=HideDuplicateItems&itemFilter(3).value=true&itemFilter(4).name=Condition&itemFilter(4).value(0)=New';

		else
			$api_request='http://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsAdvanced&SERVICE-VERSION=1.0.0&SECURITY-APPNAME=key&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&paginationInput.entriesPerPage=20&keywords='.$keyword.'&buyerPostalCode='.$zip.'&itemFilter(0).name=MaxDistance&itemFilter(0).value='.$distance.'&itemFilter(1).name=FreeShippingOnly&itemFilter(1).value='.$free.'&itemFilter(2).name=LocalPickupOnly&itemFilter(2).value='.$local.'&itemFilter(3).name=HideDuplicateItems&itemFilter(3).value=true&itemFilter(4).name=Condition&itemFilter(4).value(0)=New';
...
$id_item= $_GET['id'];
$item_api_request='http://open.api.ebay.com/shopping?callname=GetSingleItem&responseencoding=JSON&appid=key&version=967&ItemID='.$id_item.'&IncludeSelector=Description,Details,ItemSpecifics';
$item_temp=file_get_contents($item_api_request);
$items_table_json_obj= json_decode($item_temp,true);


$similar_item_api_request='http://svcs.ebay.com/MerchandisingService?OPERATION-NAME=getSimilarItems&SERVICE-NAME=MerchandisingService&SERVICE-VERSION=1.1.0&CONSUMER-ID=key&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&itemId='.$id_item.'&maxResults=8';
